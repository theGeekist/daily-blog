...
def init_topic_tables(conn: sqlite3.Connection) -> None:
    ...
    if "curator_model_route_used" not in topic_columns:
        conn.execute(
            "ALTER TABLE topic_clusters ADD COLUMN curator_model_route_used TEXT NOT NULL DEFAULT ''"
        )
    conn.commit()
...
def build_assignment_schema(claim_ids: list[str], topic_slugs: list[str]) -> dict[str, Any]:
    return {
        "type": "object",
...
def assign_topics_with_model(
    ...
    schema = build_assignment_schema(claim_ids=claim_ids, topic_slugs=allowed_slugs)
    ...
    for item in raw_assignments:
        if not isinstance(item, dict):
            raise ModelCallError("Each assignment must be an object")
        claim_id = str(item.get("claim_id", "")).strip()
        topic_slug = str(item.get("topic_slug", "")).strip()
        if claim_id in assignments:
            raise ModelCallError(f"Duplicate assignment for claim_id={claim_id}")
        if claim_id not in claim_ids:
            raise ModelCallError(f"Unknown claim_id in assignments: {claim_id}")
        if topic_slug not in allowed_slugs:
            raise ModelCallError(f"Unknown topic_slug in assignments: {topic_slug}")
        assignments[claim_id] = topic_slug

    missing = [claim_id for claim_id in claim_ids if claim_id not in assignments]
    if missing:
        raise ModelCallError(f"Missing assignments for claim_ids: {missing}")

    model_name = str(model_result.get("model_used", "")).strip() or TOPIC_LIFTER_STAGE
    return assignments, f"{TOPIC_LIFTER_STAGE}:{model_name}"
