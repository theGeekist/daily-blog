## E2E Delivery Plan (Alignment Reset)

### Why this exists

Current implementation shipped only the ingest + baseline scoring layer. The original internal goals include claim extraction, topic lifting, enrichment, editorial outputs, evaluation loops, and automation. This document resets the roadmap to the full end-to-end target.

### Goal Alignment: Planned vs Current

- Implemented now:
  - RSS ingest and persistence (`ingest_rss.py`)
  - Configurable scoring baseline (`score_rss.py`, `config/rules-engine.json`)
